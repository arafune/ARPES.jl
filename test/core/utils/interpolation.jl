...
@testset "fast_interpolate tests" begin
    ...
    @testset "2D Regular" begin
        ...
        @test _interpolate([0.0], [0.0], c0, c1, values) ≈ [1.0]
        # Test point in the middle
        @test _interpolate([0.5], [0.5], c0, c1, values) ≈ [3.0]
        # Test out of bounds (NaN)
        @test isnan(_interpolate([-1.0], [0.0], c0, c1, values)[1])
    end
    ...
    @testset "3D Regular" begin
        ...
        values = ones(2, 2, 2)
        @test _interpolate([0.5], [0.5], [0.5], c0, c1, c2, values) ≈ [1.0]
    end
    ...
    @testset "3D Irregular" begin
        ...
        @test _interpolate([1.0], [0.0], [0.0], c0, c1, c2, values) ≈ [10.0]
